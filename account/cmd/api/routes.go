...
func (app *application) routes() http.Handler {
	...
	router.HandlerFunc(http.MethodGet, "/v1/users", app.requirePermission("info:write", app.getAllUsersHandler))
	//router.HandleFunc("/users/email", app.getUserByEmailHandler).Methods("GET")
	router.HandlerFunc(http.MethodGet, "/v1/users/email", app.requirePermission("info:write", app.getUserByEmailHandler))
