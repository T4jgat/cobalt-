...
import (
	...
	"github.com/T4jgat/cobalt+/pkg/rabbitmq/rmq_rpc"
	"time"
...
func New(url, serverExchange string, router map[string]CallHandler, l logger.Interface, opts ...Option) (*Server, error) {
	...
	s := &Server{
		conn:    rmq_rpc.New(serverExchange, cfg),
...
func (s *Server) serveCall(d *amqp.Delivery) {
	...
	if err != nil {
		s.publish(d, nil, rmq_rpc.ErrInternalServer.Error())
	...
	s.publish(d, body, rmq_rpc.Success)
}
